...
export interface Category {
  ...
  slug: string
  title: string
  /** Эмодзи-иконка направления (🧘/🔥/🌿…). Опционально. */
  emoji?: string | null
  sortOrder: number
}
...
export interface Workout {
  ...
  equipment: string[]
  /** Теги «по зоне» (Шея, Плечи, Поясница, Пресс/кор…). Мультивыбор. */
  zones?: string[]
  /** Теги «по месту» (Дома, На улице, В зале, В офисе). Мультивыбор. */
  place?: string[]
  /** «По нагрузке»: мягкая / умеренная / интенсивная. Одно значение. */
  intensity?: string | null
  /** «Ограничения по нагрузке»: без прыжков, без опоры на запястья, щадяще для коленей. */
  restrictions?: string[]
...
export interface OnboardingAnswers {
  ...
  frequency?: string
  /**
   * «Бережём зоны» (CARE): чувствительные места, где нужно мягче (шея, поясница,
   * колени, запястья). Не медицинский вопрос — только ориентир для щадящего
   * подбора и заметки. Пустой массив / только «Нет, всё ок» = ограничений нет.
   */
  careAreas?: string[]
  /**
...
/** Эксклюзивная опция «Бережём зоны», означающая «ограничений нет». */
export const CARE_NONE_LABEL = 'Нет, всё ок'

/** Реально отмеченные чувствительные зоны (без «Нет, всё ок» и пустых). */
export function effectiveCareAreas(careAreas: string[] | undefined | null): string[] {
  if (!careAreas) {
    return []
  }
...
/**
 * Версия юридических документов (оферта + политика конфиденциальности).
 * Фиксируется в согласии пользователя при оплате (152-ФЗ): по этой метке видно,
 * какую редакцию он принял. Меняем строку при каждом обновлении текста.
 */
export const LEGAL_DOC_VERSION = '2026-07-21'

/** Ставка НДС для чека ЮKassa: ИП на УСН → «без НДС». */
export const RECEIPT_VAT_CODE_USN = 1

/** Простая проверка e-mail для чека (не RFC, но достаточно для UI/валидации). */
