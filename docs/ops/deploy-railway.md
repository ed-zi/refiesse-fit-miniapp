...
## Админка в Telegram (для Кати — без токенов)

Кроме веб-админки (`/admin/ui` + `ADMIN_TOKEN`) есть вход прямо из Telegram по личности — удобнее для Кати.

1. **Узнать свои Telegram ID.** В боте набрать `/admin` — если ты ещё не в списке админов, бот ответит «…Ваш Telegram ID: `12345`». Так узнаёшь ID и свой, и Катин (пусть каждый напишет `/admin`).
2. **Вписать ID в переменные** (через запятую, если несколько):
   | Сервис | Переменная | Значение |
   |--------|-----------|----------|
   | **api** | `ADMIN_TELEGRAM_IDS` | `<твой_id>,<катин_id>` |
   | **bot** | `ADMIN_TELEGRAM_IDS` | то же самое |
   | **bot** | `ADMIN_URL` | `https://<домен-api>/admin/ui` |
3. api и bot передеплоятся сами. Теперь `/admin` в боте показывает кнопку **«🛠 Открыть админку»** → открывается прямо в Telegram, вход по личности, токен вводить не нужно.

Веб-вход (`/admin/ui` + `ADMIN_TOKEN`) остаётся как запасной / для поддержки.
